...
export default async function RequestScopePage({
  ...
  const allowedTimeframes = new Set(["Immediate", "1-3 months", "Strategic"]);
  ...
  const timeframeParam = Array.isArray(resolvedSearchParams.timeframe)
    ? resolvedSearchParams.timeframe[0]
    : resolvedSearchParams.timeframe;
  const modeValue = modeParam && allowedModes.has(modeParam) ? modeParam : "remote";
  const timeframeValue =
    timeframeParam && allowedTimeframes.has(timeframeParam) ? timeframeParam : "";
  const noteValue = typeof noteParam === "string" ? noteParam.slice(0, 140) : "";
